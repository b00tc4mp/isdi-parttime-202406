...
function split(string, separator, limit = undefined) {
    ...
    // add the last segment after the last separator is found
    if (currentSegment !== "" || (currentSegment === "" && result.length < limit)) {
        result.push(currentSegment)
    }

    // if a limit is specified, the result array can´t exceed the limit
    if (limit !== undefined) {
