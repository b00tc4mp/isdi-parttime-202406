...
export default (id) => {
    ...
    return User.findById(id)
        .then((user) => {
            ...
            return Post.find({ visibility: "public" }, 'author content likes images createdAt').populate('author', 'username avatar').sort({ createdAt: -1 }).lean()
                .then(posts => {
                    return posts.map(post => {
                        ...
                        post.author.isFollowed = user.following.some(userId => userId === post.author._id);
                        ...
                        if (post.likes.length > 0) post.likes.forEach((like) => {
                            delete like._id;
                        })
